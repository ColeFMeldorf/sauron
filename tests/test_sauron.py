...
def test_chi():
    ...
    runner.fit_args_dict['z_bins'][survey] = np.arange(0, 1.4, 0.1)
    index = 1
    N_gen = datasets[f"{survey}_DUMP_IA"].z_counts(runner.fit_args_dict['z_bins'][survey])
    eff_ij = runner.calculate_transfer_matrix(survey)
    f_norm = np.sum(datasets[f"{survey}_DATA_IA_{index}"].z_counts(runner.fit_args_dict['z_bins'][survey])) / \
        np.sum(datasets[f"{survey}_SIM_IA"].z_counts(runner.fit_args_dict['z_bins'][survey]))
    n_data = datasets[f"{survey}_DATA_IA_{index}"].z_counts(runner.fit_args_dict['z_bins'][survey])
    x = np.array([2.27e-5, 1.7])
    z_centers = 0.5 * (runner.fit_args_dict['z_bins'][survey][1:] + runner.fit_args_dict['z_bins'][survey][:-1])
    logger.debug(f"Shape z_centers: {z_centers.shape}")
    null_counts = calculate_null_counts(N_gen=N_gen, true_rate_function=power_law, rate_params=x,
                                        z_bins=runner.fit_args_dict['z_bins'][survey],
                                        z_centers=z_centers)

    regression_chi = 24.422904
...
def test_cc_decontam():
    ...
    for i in range(50):
        ...
        # plt.plot(runner.datasets[f"{survey}_SIM_IA"].z_counts(runner.z_bins, prob_thresh = 0.5),ls = "--", label='Sim IA Counts Cut')
        # plt.plot(runner.datasets[f"{survey}_SIM_CC"].z_counts(runner.z_bins, prob_thresh = 0.5),ls = "--" ,label='Sim CC Counts Cut')
        # plt.plot(runner.datasets[f"{survey}_SIM_ALL"].z_counts(runner.z_bins, prob_thresh = 0.5),ls = "--", label='Sim All Counts Cut')
        # plt.legend()
        ...
        # plt.subplot(1,2,2)
        # plt.plot(runner.datasets[f"{survey}_DATA_IA_{index}"].z_counts(runner.z_bins), label='Sim IA Counts')
        # plt.plot(runner.datasets[f"{survey}_DATA_CC_{index}"].z_counts(runner.z_bins), label='Sim CC Counts')
        # plt.plot(runner.datasets[f"{survey}_DATA_ALL_{index}"].z_counts(runner.z_bins), label='Sim All Counts')
        # plt.plot(runner.datasets[f"{survey}_DATA_IA_{index}"].z_counts(runner.z_bins, prob_thresh = 0.5),ls = "--", label='Sim IA Counts Cut')
        # plt.plot(runner.datasets[f"{survey}_DATA_CC_{index}"].z_counts(runner.z_bins, prob_thresh = 0.5),ls = "--", label='Sim CC Counts Cut')
        # plt.plot(runner.datasets[f"{survey}_DATA_ALL_{index}"].z_counts(runner.z_bins, prob_thresh = 0.5)*bias_cor, color = "k", lw= 3,ls = "--", label='Sim All Counts Cut')
        # plt.legend()
        # plt.savefig(pathlib.Path(__file__).parent / f"aaaa_test_cc_decontam_simcounts_{index}.png")
...
def test_cc_decontam_small():
    ...
    for i in range(n_trials):
        ...
        plt.plot(runner.datasets[f"{survey}_DATA_CC_{index}"].z_counts(runner.z_bins), label='Sim CC Counts')
        plt.plot(runner.datasets[f"{survey}_DATA_ALL_{index}"].z_counts(runner.z_bins), label='Sim All Counts')
        plt.plot(runner.datasets[f"{survey}_DATA_IA_{index}"].z_counts(runner.z_bins, prob_thresh = PROB_THRESH),ls = "--", label='Sim IA Counts Cut')
        plt.plot(runner.datasets[f"{survey}_DATA_CC_{index}"].z_counts(runner.z_bins, prob_thresh = PROB_THRESH),ls = "--", label='Sim CC Counts Cut')
        plt.plot(runner.datasets[f"{survey}_DATA_ALL_{index}"].z_counts(runner.z_bins, prob_thresh = PROB_THRESH)*bias_cor, color = "k", lw= 3,ls = "--", label='Sim All Counts Cut')
        plt.legend()
        plt.savefig(pathlib.Path(__file__).parent / f"aaaa_test_cc_decontam_simcounts_{index}.png")
