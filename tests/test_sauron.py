...
def test_regression_pz_5datasets():
    """In this test, we simply test that nothing has changed significantly.
       This is using CC decontam and realistic data. Photo Zs.
...
def test_perfect_recovery_multisurvey():
    ...
    cmd = ["python", str(sauron_path), str(config_path), "-o", str(outpath), "--cheat_cc",
           "--no-sys_cov", "--no-sanity-check"]
    result = subprocess.run(cmd, capture_output=False, text=True)
    if result.returncode != 0:
        raise RuntimeError(
            f"Command failed with exit code {result.returncode}\n"
...
def test_cc_decontam_small():
    ...
    for i in range(n_trials):
        ...
        plt.plot(runner.datasets[f"{survey}_DATA_ALL_{index}"].z_counts(runner.z_bins,
                                                                        prob_thresh=PROB_THRESH)*bias_cor,
                 color="k", lw=3, ls="--", label="Data All Counts Cut w/ BCor")
        plt.yscale("log")
        plt.legend()
