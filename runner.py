...
class sauron_runner():
    ...
    def generate_chi2_map(self, survey, index, n_samples=50):
        ...
        # these need to have indices soon
        chi2_result = chi2((2.27e-5, 1.7), fit_args_dict['null_counts'][survey][index], fit_args_dict['f_norm'][survey],
                           z_centers,
        ...
        for i, a in enumerate(np.linspace(2.0e-5, 2.6e-5, n_samples)):
            for j, b in enumerate(np.linspace(1.4, 2.0, n_samples)):
                ...
                chi2_result = chi2(values, fit_args_dict['null_counts'][survey][index], fit_args_dict['f_norm'][survey],
                                   z_centers,
                                   fit_args_dict['eff_ij'][survey],
                                   fit_args_dict['n_data'][survey][index],
                                   self.rate_function,
